...
**`<SeoKit>`{lang="html"}, `useSeoKit()`{lang="ts"} removed**: these APIs set up all the default meta and module configuration for you.
...
The behavior for environment variables hasn't changed. It's recommended to read [how site config works](/docs/site-config/guides/how-it-works) for
...
- `defineOgImageStatic()`{lang="ts"} is deprecated, use `defineOgImage()`{lang="ts"} (default behavior is to cache), if you want to be verbose you can use `defineOgImageCached()`{lang="ts"} or `<OgImageCached />`{lang="html"}
