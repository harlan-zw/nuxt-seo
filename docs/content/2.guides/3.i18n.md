...
All modules use the translated values automatically: meta tags, Schema.org, OG images, and anywhere else site name or description appears.
...
Schema.org creates a `WebSite` entity per locale with `workTranslation` linking between them:
...
OG Image generates images per locale, using the locale-specific site name and description as default props.
