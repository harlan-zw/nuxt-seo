...
The v3 release is here, and it's a complete rewrite of the module to improve stability and DX. I think you're going
to love it!

While I was happy with the v2 release, it was clear through the issues open, that it still had some problems to solve.
...
- Custom Nitro prerenderer config

As v2 has aged, many of these features have become more stable, and hacks are able be replaced with more stable solutions.

There are also a few developer experience issues to solve:
- Only a single template out-of-the-box, hard to customize
- Hard to figure out compatibility of the module with different providers

These will be explored further in the release notes.
...
In v3 we switch to using the [Iconify API](https://iconify.design/docs/api/) which gives us access to to the following emoji families:
...
The default emoji family has been switched to `noto`. This is because it has the most complete set of emojis and
looks the best (imo).

### 🖼️ JPEG OG Images

In certain circumstances, using a `png` image is not ideal. They are usually quite large, and
they don't support transparency.

For Chromium based rendering, `jpeg` images will now be rendered by default. 

For Satori, a `sharp` dependency will be used to generate `jpeg`'s when the extension is set to `jpeg` or `jpg`. This
does come as a trade-off on rendering time and compatibility, but can be useful in certain circumstances.

For runtime JPEG's, Sharp is working on full WASM support, in the mean-time it will only work on some Node based providers.
See the [compatibility guide](/og-image/guides/compatibility) for more details.
...
can now natively handle the prerendering of screenshots without bundling the `playwright-core` dependency.

Previously, it relied on running a scripts after the prerendering was finished, which meant you couldn't take advantage of 
multithreaded prerendering.
...
- `runtimeBrowser` has been renamed to `runtimeChromium`
