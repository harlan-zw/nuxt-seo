...
The pull request workflow builds the base and head commits on one runner. Its comment leads with a compact workload comparison. Module costs and current SSR hotspots stay collapsed until needed. CPU changes need 5% plus measured uncertainty. Memory changes need both 3% and 16 KiB. The artifact preserves both result sets, full caller paths, and all raw profiles.
